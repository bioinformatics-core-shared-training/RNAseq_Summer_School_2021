...
The first step in our analysis is to quantify the gene expression. One way is to
align our raw reads to the reference genome using a short read aligner, such as
STAR, and then  to look at the alignments and use information about where genes
are located on the genome to count the number of reads coming from each gene.
This method is shown in the [extended materials using the `featureCounts`
tool](S8_Read_Counts_with_SubRead.html).
...
STAR, we will need a reference to map the reads to, however, in this case we
...
can run `Salmon` we need to create an index, just as we did for `STAR`. For
...
these so that Salmon will know which are which. In our case we are just using
...
>````
> 1. Create concatenated trancriptome/genome reference file. The two files that
> we need are in the `references` directory.
>```{bash eval=FALSE}
...
>       * `-l A` - Salmon needs to use some information about the **l**ibrary 
...
```
`multiqc` will then scan all the files in the directory to find metrics files it
recognises. We will add a couple of additionaly options to control the output
directory and the name of the report.
...
> 1. Run `multiqc` on the `salmon` directory and create a report called 
> `Salmon_quantification_report` in the `salmon` directory.
```{bash eval = FALSE}
...
our differential gene expression analysis in R, we will want to summarize this
